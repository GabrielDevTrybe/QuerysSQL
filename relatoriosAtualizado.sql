...
WHERE OwnerId = 20 and Code like '%VP_01_BRASILIA_%';

-- o intuito é eu saber a quantidade que vai na coluna "Novos".
...
-- OBS: Enquanto eu descomento a linha de cima, posso deixar descomentado, nao preciso comentar de novo.
SELECT COUNT(ifi.`Identity`)  
FROM identity_FanId ifi 
	LEFT JOIN event_tickets et1 ON ifi.`Identity` = et1.`identity` AND et1.eventCode = 'VP_01_BRASILIA_01'
	LEFT JOIN event_tickets et2 ON ifi.`Identity` = et2.`identity` AND et2.eventCode = 'VP_01_BRASILIA_02'
	LEFT JOIN event_tickets et3 ON ifi.`Identity` = et3.`identity` AND et3.eventCode = 'VP_01_BRASILIA_03'
	...
	INNER JOIN event_tickets et6 ON ifi.`Identity` = et6.`identity` AND et6.eventCode = 'VP_01_CUIABA_06'
...
WHERE et.eventCode like '%VP_01_BRASILIA_%'
...
-- Os criados tem que bater com a quantidade total de faixa etaria por linha
-- o count do primeiro Select só vai las linhas onde tem "Geral"
